...
export function SearchInput({id, value, placeholder, disabled, error={hasError: false, msg: ""}, onChange}:BaseInputProps) {
    const inputClasses = `${defaultStyles} ${error.hasError ? errorStyles : disabled ? disabledStyle : ''} pl-11`;

    return (
        <div className="flex items-center relative">
            <input id={id} type="text" value={value} className={inputClasses} placeholder={placeholder} onChange={(e) => onChange?.(e.target.value)}/>
